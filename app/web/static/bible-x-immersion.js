...
(function () {
  ...
  function injectVerseAiDock() {
    ...
    if (!document.getElementById("bxVerse248Dock")) {
      ...
      o8.textContent = `.bx-verse-ai-dock{box-sizing:border-box;max-width:100%!important;flex-wrap:wrap;gap:10px 14px;margin:12px auto 4px;padding:9px 12px}.bx-verse-ai-dock>div{min-width:0}.bx-verse-ai-dock nav{flex-wrap:wrap;gap:6px;justify-content:center}.bx-verse-ai-dock nav button{white-space:nowrap;height:auto;min-height:36px;padding:6px 11px}.bx-verse-ai-dock b{font-size:.9rem;line-height:1.2}.bx-verse-ai-dock small{display:none}@media(max-width:680px){.bx-verse-ai-dock{flex-direction:column;align-items:stretch}.bx-verse-ai-dock nav{justify-content:flex-start}.bx-verse-ai-dock b{font-size:.9rem}.bx-verse-ai-dock small{display:block;font-size:.76rem;line-height:1.3;margin-top:2px}}`;
  ...
  /* ---- 5.4.249 — ordem da Bíblia para navegar a biblioteca inteira -------
     "João 3:16" -> [indice do livro, capitulo, versiculo]. Livro desconhecido
     vai para o fim, para nunca sumir da lista. */
  function bxRefOrder(value) {
    const texto = String(value || "").trim();
    const m = texto.match(/^(.*?)\s+(\d+)(?::(\d+)(?:-(\d+))?)?$/);
    if (!m) return [900, 0, 0];
    let idx = 800;
    try {
      if (typeof BOOK_NAMES !== "undefined" && Array.isArray(BOOK_NAMES)) {
        const achou = BOOK_NAMES.findIndex(b => norm(b) === norm(m[1]));
        if (achou >= 0) idx = achou;
      }
    } catch (_) {}
    return [idx, Number(m[2]) || 0, Number(m[3]) || 0];
  }
  function bxOrdenarPorBiblia(rows) {
    return (rows || []).slice().sort((a, b) => {
      const ra = bxRefOrder(a && a.reference);
      const rb = bxRefOrder(b && b.reference);
      for (let i = 0; i < 3; i += 1) if (ra[i] !== rb[i]) return ra[i] - rb[i];
      return String((a && a.createdAt) || "").localeCompare(String((b && b.createdAt) || ""));
    });
  }
  function bxMediaAllIds(videoOnly) {
    const out = [];
    for (let i = 0; i < bxMediaIdx.length; i += 1) {
      const en = bxMediaIdx[i];
      if (!en || !en.hasData || !en.id) continue;
      if (videoOnly === true && !en.video) continue;
      if (videoOnly === false && en.video) continue;
      out.push(en.id);
    }
  ...
  function bxAbrirBibliotecaInteira(ref, videoOnly, eyebrow, emptyMsg) {
    bxMediaIdxEnsure(() => {
      bxReadMediaByIds(bxMediaAllIds(videoOnly), (rows) => {
        const lista = bxOrdenarPorBiblia(videoOnly ? passageRowsToVideos(rows) : passageRowsToImages(rows));
        if (!lista.length) { bxMediaToast(emptyMsg); return; }
        /* começa na mídia DESTE versículo; o resto da biblioteca fica antes e
           depois dela, para as setas não pararem no fim do versículo. */
        const alvo = lista.findIndex(row => [row.reference].concat(Array.isArray(row.relatedReferences) ? row.relatedReferences : [])
          .some(valor => mediaRefMatches(valor || "", ref)));
        const chave = alvo >= 0 ? String(lista[alvo].id || lista[alvo].key || "") : "";
        if (videoOnly) openPassageVideosDirect(lista, "", eyebrow, chave);
        else openPassageGallery(lista, "", eyebrow, chave);
      });
    });
  }
  ...
  function openVerseImagesDirect(ref) {
    bxAbrirBibliotecaInteira(ref, false, `MÍDIA X • IMAGENS DA PASSAGEM • ${ref}`,
      `Nenhuma imagem salva para ${ref} ainda. Gere uma no Gerador de IA.`);
  }

  function openVerseVideosDirect(ref) {
    bxAbrirBibliotecaInteira(ref, true, `MÍDIA X • VÍDEOS DA PASSAGEM • ${ref}`,
  ...
  function ensureVerseAiButtons(tools, verse) {
    ...
    /* 5.4.249 — GOOGLE VIEW abaixo de CADA versículo. Verde suave quando o
       texto cita um lugar do nosso catálogo (bible-x-lugares.js) e o clique
       cai JÁ NA RUA, que é onde o Street View tem as setas brancas de andar;
       branco quando não cita, igual aos botões das imagens geradas — e aí a
       janela abre com o campo de busca pronto e o aviso dizendo que este
       versículo não tem lugar no catálogo, em vez de inventar um. */
    ...
    const textoVerso = (() => { const n = verse.querySelector(".lmx-bible-v3-text, [data-bx-verse-text]"); return n ? String(n.innerText || n.textContent || "").trim() : ""; })();
    const gChave = `${ref}|${textoVerso.length}`;
    if (gBtn.dataset.bxGmapKey !== gChave) {
      gBtn.dataset.bxGmapKey = gChave;
      const achar = window.BibleXLugaresAchar;
      const lugar = (achar && textoVerso) ? achar(textoVerso) : null;
      const lista = window.BibleXLugares || [];
      gBtn.dataset.bxGmapIdx = lugar ? String(lista.indexOf(lugar)) : "";
      gBtn.classList.toggle("is-lugar", !!lugar);
      gBtn.textContent = lugar ? (lugar[0] || "🗺") + " " + lugar[1] : "🗺 Google View";
      const anda = !!(lugar && window.BibleXLugaresTemRua && window.BibleXLugaresTemRua(lugar));
      gBtn.title = lugar
        ? lugar[1] + (anda ? " — dá para ANDAR pela rua no Street View" : " — o Google não tem carro de rua aqui; abre no satélite") + " · toque para abrir"
        : "Este versículo não cita um lugar do nosso catálogo — abre o Google View com a busca pronta";
    }
    if (!gBtn.dataset.bxGmapBound) {
      gBtn.dataset.bxGmapBound = "1";
      gBtn.addEventListener("click", (ev) => {
        ev.preventDefault(); ev.stopImmediatePropagation();
        const janela = window.BibleXGoogleView;
        if (!janela) return;
        const lista = window.BibleXLugares || [];
    ...
    if (!document.getElementById("bxVerse249Gmap")) {
      ...
      gs.id = "bxVerse249Gmap";
      gs.textContent = `#bOut .lmx-bible-v3-tools button[data-bx-verse-gmap]{border:1px solid #d3e2ea!important;border-radius:10px;padding:6px 11px!important;background:#f7fafc!important;color:#0b2a3a!important;font-weight:800;font-size:.72rem;line-height:1.15;min-height:26px;display:inline-flex;align-items:center;gap:5px;cursor:pointer;box-shadow:none!important}#bOut .lmx-bible-v3-tools button[data-bx-verse-gmap]:hover{background:#e7f4f6!important}#bOut .lmx-bible-v3-tools button[data-bx-verse-gmap].is-lugar{border-color:rgba(76,175,110,.9)!important;color:#08381d!important;background:linear-gradient(180deg,#ddf6e4,#a9e7c0)!important;box-shadow:0 0 0 1px rgba(76,175,110,.28),0 0 12px rgba(96,206,140,.35)}#bOut .lmx-bible-v3-tools button[data-bx-verse-gmap].is-lugar:hover{background:linear-gradient(180deg,#d0f2da,#96dfb1)!important}@media(max-width:680px){#bOut .lmx-bible-v3-tools button[data-bx-verse-gmap]{padding:5px 9px!important;font-size:.7rem}}`;
