...
(function () {
  ...
  var estado = {
    ...
    mostrarChave: false,
    /* Ligar o 🗺 Google é entrar num MODO: as outras fontes saem da frente e
       ficam guardadas aqui para voltarem quando o modo for desligado. */
    fontesAntesGoogle: null,
    avisoGoogle: ""
  };
  ...
  /* A lista vem do CATÁLOGO ÚNICO (bible-x-lugares.js) — o mesmo que alimenta
     o botão abaixo de cada versículo e os passeios prontos do visualizador.
     Eram 18 lugares escritos à mão aqui e uma segunda cópia lá; agora é uma
     lista só, e ninguém fica com números diferentes.

     Ordem: primeiro os que dão para ANDAR (o carro do Google passou na rua, e
     é isso que faz aparecer a seta branca no chão). Os que só têm mapa e
     satélite — Síria, Iraque, Irã, mar aberto — vêm depois, no fim. */
  var LUGARES_GOOGLE = (function () {
    var todos = (window.BibleXLugares || []).slice();
    if (!todos.length) return [];
    var tem = window.BibleXLugaresTemRua || function () { return false; };
    var anda = todos.filter(tem);
    var soMapa = todos.filter(function (l) { return !tem(l); });
    return anda.concat(soMapa);
  })();
  ...
  function cartoesGoogle(consulta) {
    ...
    var notas = LUGARES_GOOGLE.map(function (lugar, i) {
      ...
      var nome = semAcento(String(lugar[1]) + " " + String(lugar[6] || "").replace(/;/g, " ")).toLowerCase();
      var nota = 0;
    ...
    return escolhidos.map(function (x) {
      var lugar = x.lugar;
      /* Lugar sem rua do Google (Síria, Iraque, Irã, mar aberto): cai no
         sítio. Sem isto o endereço sairia literalmente "null,null". */
      var temRua = lugar[4] !== null && lugar[4] !== undefined;
      var ruaLat = temRua ? lugar[4] : lugar[2];
      var ruaLon = temRua ? lugar[5] : lugar[3];
      return {
        id: "gmap-" + x.indice,
        fonte: "🗺 Google View",
        titulo: lugar[1],
        ...
        pagina: "https://www.google.com/maps/@?api=1&map_action=pano&viewpoint=" + ruaLat + "," + ruaLon,
        largura: 0,
        altura: 0,
        ...
        gmap: { lat: lugar[2], lon: lugar[3], ruaLat: lugar[4], ruaLon: lugar[5],
                nome: lugar[1], emoji: lugar[0], indice: x.indice, semRua: !temRua }
      };
    });
  }
  ...
  function css() {
    ...
    style.textContent = [
      ...
      /* 🗺 sozinho na fileira = MODO Google View (só navegação de rua) */
      ".bxpub-chip.is-so{border-color:#ffd479;background:rgba(255,212,121,.2);color:#ffe6a8;box-shadow:0 0 0 1px rgba(255,212,121,.5) inset}",
      ".bxpub-aviso.is-google{border-color:rgba(255,212,121,.5);background:rgba(255,212,121,.08);color:#ffe6a8}",
  ...
  function desenhar() {
    ...
    $$("[data-bxpub-fonte]", overlay).forEach(function (b) {
      ...
      b.classList.toggle("is-so", id === "google" && !!estado.fontes.google
        && !estado.fontes.wikimedia && !estado.fontes.openverse && !estado.fontes.pexels);
    });
    ...
    if (avisos) {
      var linhas = [];
      if (estado.avisoGoogle) linhas.push('<p class="bxpub-aviso is-google">' + esc(estado.avisoGoogle) + "</p>");
      linhas = linhas.concat(estado.avisos.map(function (a) { return '<p class="bxpub-aviso">' + esc(a) + "</p>"; }));
  ...
  function abrir() {
    ...
    overlay.addEventListener("click", function (ev) {
      ...
      if (fonte) {
        ...
        /* ---- 5.4.249 — 🗺 GOOGLE VIEW É MODO, NÃO FONTE ------------------
           Um toque no 🗺 DESMARCA todas as outras fontes e deixa só a
           navegação de rua: a tela passa a mostrar apenas os lugares por onde
           dá para ANDAR (Street View), sem 30 fotos de acervo no meio. Um
