(function(){
  ...
  let _raf=0;function mountAll(){_raf=0;Object.keys(cfg).forEach(mount)}function init(){const observer=new MutationObserver(()=>{if(!_raf)_raf=requestAnimationFrame(mountAll)});observer.observe(document.body,{childList:true,subtree:true});mountAll();document.addEventListener('biblex:pagechange',mountAll)}
