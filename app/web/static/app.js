...
const VISUAL_DEFAULT={layout:"classico",theme:"dark",accent:"#d6b25e",mobileLayout:"mobile-clean"};
...
function applyVisual(v=activeVisual()){const root=document.documentElement;if(v.theme==="system")v={...v,theme:"dark"};if(v.layout==="compacto")v={...v,layout:"modernox"};root.dataset.layout=v.layout;root.dataset.theme=v.theme;root.dataset.mobileLayout=v.mobileLayout||"mobile-clean";root.style.setProperty("--accent",v.accent);root.style.setProperty("--gold",v.accent);updateNavIcons(v.layout);}
const NAV_META={
...
const views={
 dashboard(){const s=projectStats(),h=Store.get("history",[])[0];return `<div class="hero"><h1>LOGOS MASTER X 3.7.3</h1><p>Preparação bíblica • IA multiprovedor • DNA K7 • uso local + nuvem</p></div>
<div class="grid">
...
async function checkFrontendVersion(){
 try{
   const r=await fetch((IS_LOCAL_HOST?PRODUCTION_VERSION_URL:publicAsset("version.json"))+"?t="+Date.now(),{cache:"no-store",headers:{"Cache-Control":"no-cache"}});
   if(!r.ok)return;
...
function installUpdateControls(){const top=document.querySelector('.top');if(top&&!$('#updateCenterBtn')){const b=document.createElement('button');b.id='updateCenterBtn';b.className='btn secondary update-center-top';b.textContent=IS_LOCAL_HOST?'🚀 Publicar / Atualizar':'🔄 Atualizações';b.onclick=openUpdateCenter;top.insertBefore(b,$('#status'));}if(!$('#updateDock')){const d=document.createElement('div');d.id='updateDock';d.className='update-dock';d.innerHTML=`<span>LOGOS ${APP_BUILD_VERSION}</span><button id="updateDockCheck">🔄 Verificar</button>${IS_LOCAL_HOST?'<button id="updateDockPublish">🚀 Publicar</button><button id="updateDockAuto">⚙ Auto</button>':'<button id="updateDockNow">⬆ Atualizar</button>'}`;document.body.appendChild(d);$('#updateDockCheck').onclick=checkFrontendVersion;$('#updateDockPublish')&&($('#updateDockPublish').onclick=openUpdateCenter);$('#updateDockAuto')&&($('#updateDockAuto').onclick=()=>toggleAutoPublish(!Store.get('autoPublish',false)));$('#updateDockNow')&&($('#updateDockNow').onclick=openUpdateCenter);}if(IS_LOCAL_HOST&&Store.get('autoPublish',false))toggleAutoPublish(true);}
