...
const VISUAL_DEFAULT={layout:"classico",theme:"dark",accent:"#d6b25e",mobileLayout:"mobile-pro",appIcon:"fixed",palette:"aurora"};
const COLOR_THEMES={
  aurora:{id:"aurora",name:"Aurora Ciano + Dourado",thumb:"/static/brand/themes/aurora-thumb.jpg",home:"/static/brand/themes/aurora-body.jpg",accent:"#22e1df"},
  gold:{id:"gold",name:"Dourado Preto",thumb:"/static/brand/themes/gold-thumb.jpg",home:"/static/brand/themes/gold-body.jpg",accent:"#e2b13f"},
  forest:{id:"forest",name:"Verde Épico",thumb:"/static/brand/themes/forest-thumb.jpg",home:"/static/brand/themes/forest-body.jpg",accent:"#9de72f"}
};
...
function applyVisual(v=activeVisual()){const root=document.documentElement;if(v.theme==="system")v={...v,theme:"dark"};if(v.layout==="compacto"||v.layout==="modernox"||v.layout==="moderno")v={...v,layout:"clean"};const palette=COLOR_THEMES[v.palette]||COLOR_THEMES.aurora;const accent=v.layout==="clean"?"#08c6c9":palette.accent;root.dataset.layout=v.layout;root.dataset.theme="dark";root.dataset.mobileLayout=v.mobileLayout||"mobile-pro";root.dataset.palette=palette.id;root.style.setProperty("--accent",accent);root.style.setProperty("--gold",accent);syncManifestIcon();updateNavIcons(v.layout);const homeImg=document.querySelector('.reference-body-img');if(homeImg&&v.layout==="classico")homeImg.src=palette.home+"?v=theme3";}
const NAV_META={
...
function appearancePanel(){const v=activeVisual();const themeCards=Object.values(COLOR_THEMES).map(t=>`<button class="theme-card ${v.palette===t.id?"active":""}" data-palette="${t.id}"><img src="${t.thumb}?v=theme3" alt="${t.name}"><span><strong>${t.name}</strong><small>Home, menus e páginas internas</small></span></button>`).join("");return `<div class="appearance-backdrop" id="appearanceBackdrop" role="dialog" aria-modal="true"><div class="appearance-panel" id="appearancePanel"><div class="appearance-head"><h3>🎨 Aparência</h3><button class="btn secondary" id="appearanceClose">✕ Fechar</button></div><p class="muted">Escolha uma identidade de cor. A Home usa a arte aprovada e as páginas internas acompanham os mesmos degradês e brilhos.</p><label>✨ Temas de cores</label><div class="theme-card-grid">${themeCards}</div><label>🖥️ Estrutura no PC</label><div class="visual-options">${[["classico","🏛️ Clássico — imagem completa"],["clean","✦ Clean"]].map(([x,l])=>`<button class="visual-choice ${v.layout===x?"active":""}" data-layout="${x}">${l}</button>`).join("")}</div><label>📱 Estrutura no celular / Android</label><div class="visual-options">${[["mobile-pro","⚡ Pro — padrão"],["mobile-clean","✦ Clean"]].map(([x,l])=>`<button class="visual-choice ${v.mobileLayout===x?"active":""}" data-mobile-layout="${x}">${l}</button>`).join("")}</div><div class="fixed-icon-card theme-icon-preview"><img src="${(COLOR_THEMES[v.palette]||COLOR_THEMES.aurora).thumb}?v=theme3" alt="Miniatura do tema"><div><strong>Miniatura seletiva do tema</strong><small>O ícone oficial do APP continua preservado.</small></div></div><div class="row appearance-actions"><button class="btn primary" id="visualSave">💾 Salvar tema</button><button class="btn secondary" id="visualReset">↩ Restaurar padrões</button><button class="btn secondary" id="visualCloseBottom">✕ Fechar</button></div></div></div>`;}
...
function themeHomeAsset(){const p=COLOR_THEMES[activeVisual().palette]||COLOR_THEMES.aurora;return p.home+"?v=theme3";}
