...
const views={
 dashboard(){const s=projectStats();return `<section class="home-brand-hero"><img src="/static/brand/${activeVisual().layout==='moderno'?'modern-home':'classic-home'}.png" alt="LOGOS MASTER X DNA K7"><div class="home-brand-overlay"><strong>LOGOS MASTER X</strong><span>DNA K7 • Bíblia • Estudo • Pregação</span><button class="btn primary" data-go="studio">▶ ACESSAR STUDIO</button></div></section><div class="home-quick"><button data-go="studio">✒️<b>Studio</b><small>Gerar mensagem</small></button><button data-go="library">📚<b>Biblioteca</b><small>${s.library} materiais</small></button><button data-go="projects">📂<b>Projetos</b><small>Organizar</small></button><button data-go="pulpit">🎙️<b>Modo Púlpito</b><small>Preparação final</small></button><button data-go="aihub">✦<b>AI HUB</b><small>Provedores e modelos</small></button></div><div class="home-panels"><div class="card"><h3>📲 LOGOS no seu dispositivo</h3><p class="muted">Instale como aplicativo e use o layout adaptável para celular.</p><button class="btn primary" id="installPwaHome">📲 Instalar como APP</button><button class="btn secondary" data-go="appearance">🎨 Tema e ícone</button></div><div class="card"><h3>ⓘ O propósito</h3><p class="muted">Da antiga fita K7 ao ambiente digital: tecnologia para servir estudo, preparação e comunicação bíblica.</p><button class="btn secondary" data-go="about">Conhecer a história →</button></div><div class="card"><h3>➕ Personalize</h3><p class="muted">Crie páginas próprias e transforme-as em novos atalhos do LOGOS.</p><button class="btn secondary" data-go="custompages">Criar página</button></div></div>`},
...
async function render(view){
 ...
 if(view==="dashboard"){$("#installPwaHome")?.addEventListener("click",installPwa);}
...
const top2=document.querySelector(".top");if(top2&&!document.querySelector("#appearanceBtn")){const b=document.createElement("button");b.id="appearanceBtn";b.className="btn secondary appearance-trigger";b.textContent="🎨 Aparência";b.onclick=openAppearance;top2.insertBefore(b,document.querySelector("#status"));}
render("dashboard");
   await checkApi();
 }catch(e){
