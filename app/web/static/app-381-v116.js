...
async function render(view){
 ...
 if(view==="studio"){
   ...
   if(Number(Store.get("studioStep",1))===6){
    const getProc=()=>Object.assign({status:"idle",progress:0,phase:0,message:"Preparando geração...",started:null,finished:null,engine:"",provider:"",model:"",quality:null,error:""},Store.get("studioProcessing",{}));
    ...
    const runStudioGeneration=async()=>{
      ...
      p=setProc({status:"running",progress:6,phase:0,message:"Consolidando DNA e estrutura...",started:new Date().toISOString(),finished:null,error:""});paintProc(p);
      ...
      }catch(e){clearInterval(timer);setProc({status:"error",progress:Math.max(10,getProc().progress||0),message:"A geração foi interrompida.",finished:new Date().toISOString(),error:e?.message||"Falha inesperada"});render("studio")}
    };
    ...
    $("#dna6Back")?.addEventListener("click",()=>{if(getProc().status==="running")return;Store.set("studioStep",5);render("studio")});
