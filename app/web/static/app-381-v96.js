...
<div class="mobile-reference-home" aria-label="LOGOS MASTER X — Home adaptada para celular">
 ...
 return `<div class="quick-generator-pro">
  ...
  <div class="quick-layout"><main>
   ...
   <button class="btn primary quick-run" id="quickRun">⚡ GERAR / EXECUTAR <small>Gerar mensagem agora</small></button>
...
async function render(view){
 ...
 if(view==="quick"){
   ...
   const setTab=t=>{tab=t;$$('[data-quick-tab]').forEach(b=>b.classList.toggle('active',b.dataset.quickTab===t));if($('#quickMessagePane'))$('#quickMessagePane').hidden=t!=="message";if($('#quickTipsPane'))$('#quickTipsPane').hidden=t!=="tips";};
   ...
   $('#quickRun').onclick=async()=>{
     ...
     if(tab==='tips'){
       ...
       const oldMode=App.aiMode;App.aiMode=mode;const r=await runCommand('ESTUDAR',d);App.aiMode=oldMode;App.lastStudioText=r.text;out.textContent=r.text;Store.push('history',{id:Date.now(),cmd:'DICA RÁPIDA',input:d,result:r.text,created:new Date().toISOString()});return;
     }
     const d=fd();if(!d.text){out.textContent='Digite o texto bíblico ou tema.';return}
     const cmd=$('#cmd').value||'SERMÃO';out.textContent='Gerando...';
     const oldMode=App.aiMode;App.aiMode=mode;const started=performance.now();const r=await runCommand(cmd,d);App.aiMode=oldMode;const secs=Math.round((performance.now()-started)/1000);saveModeTime(mode,secs);App.lastStudioText=r.text;out.textContent=r.text;Store.push('history',{id:Date.now(),cmd,input:d,result:r.text,provider:r.provider||'',quality:r.quality||null,created:new Date().toISOString()});
   };
