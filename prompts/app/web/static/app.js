...
let mobileLoadingProgress=100;let mobileLoadingTimer=null;const mobileLoadingStartedAt=performance.now();
function startMobileLoading(){return;}
...
<div class="mobile-reference-home" aria-label="LOGOS MASTER X — Home adaptada para celular">
 ...
 const scoreColor=(score)=>{
   const v=Math.max(0,Math.min(110,Number(score)||0));
   const h=Math.round((v/110)*120);
   return `hsl(${h} 92% 54%)`;
 };
 const profileGraph=(x)=>{
   const pct=Math.max(0,Math.min(110,Number(x.score)||0));
   const gid=`scoreGrad-${x.id}`;
   const endColor=scoreColor(pct);
   if(graphMode==="line"){
     const base=[28,44,35,55,43,62,49,68,58,76,65,84];
     const pts=base.map((v,i)=>`${8+i*10},${58-Math.min(52,Math.max(5,(v*pct/110)*.62))}`).join(" ");
     return `<div class="dna-score-viz dna-score-line" style="--score-color:${endColor}"><svg viewBox="0 0 120 64" aria-hidden="true"><defs><linearGradient id="${gid}" x1="0" y1="0" x2="1" y2="0"><stop offset="0%" stop-color="#ff3030"/><stop offset="50%" stop-color="#ffd51f"/><stop offset="100%" stop-color="#2ee85d"/></linearGradient></defs><polyline points="${pts}" fill="none" stroke="url(#${gid})" stroke-width="2.8" stroke-linecap="round" stroke-linejoin="round"/><g>${pts.split(' ').map((pt,i)=>{const [cx,cy]=pt.split(',');return `<circle cx="${cx}" cy="${cy}" r="2.25" fill="${i===11?endColor:'#e8f1f7'}"/>`}).join('')}</g></svg></div>`;
   }
   return `<div class="dna-score-viz dna-score-gauge" style="--score-color:${endColor}"><svg viewBox="0 0 120 72" aria-hidden="true"><defs><linearGradient id="${gid}" x1="0" y1="0" x2="1" y2="0"><stop offset="0%" stop-color="#ff3030"/><stop offset="50%" stop-color="#ffd51f"/><stop offset="100%" stop-color="#2ee85d"/></linearGradient></defs><path class="gauge-track" d="M10 62 A50 50 0 0 1 110 62" pathLength="110"/><path class="gauge-value" d="M10 62 A50 50 0 0 1 110 62" pathLength="110" stroke="url(#${gid})" stroke-dasharray="${pct} 110"/></svg></div>`;
 };
 const charRows=[
