...
def _git(*args,timeout=120):
    ...
    if r.returncode!=0:
        raise HTTPException(500,(r.stderr or r.stdout or "Falha Git").strip())
...
def dev_status(request:Request):
    ...
    return {"ok":True,"repository":True,"repairable":False,"branch":_git("branch","--show-current") or "main","dirty":bool(status),"files":[x for x in status.splitlines() if x],"commit":commit.stdout.strip() if commit.returncode==0 else "—","origin":_git_origin_url()}
